...
import { getDirectConversationById } from "@/server-actions/conversations";
import type { InboxDirect } from "@/types/chat";
import LoadingChatArea from "../../_components/LoadingChatArea";
import { useParams } from "next/navigation";

export default function DirectConversationPage() {
  const { id } = useParams<{ id: string }>();
  const [conversation, setConversation] = useState<InboxDirect | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchConv = async () => {
      for (let attempt = 0; attempt < 5 && !canceled; attempt++) {
        const conv = await getDirectConversationById(id);
        if (conv && conv.id) {
